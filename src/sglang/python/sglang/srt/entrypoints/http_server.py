...
@app.api_route("/shutdown", methods=["POST"])
async def shutdown(request: Request):
    graceful_q = request.query_params.get("graceful", "true").lower()
    graceful = graceful_q in ("1", "true", "yes", "y")
    logger.info(f"Received shutdown request graceful={graceful}")
    _global_state.tokenizer_manager.initiate_shutdown(graceful=graceful)
    return Response(
