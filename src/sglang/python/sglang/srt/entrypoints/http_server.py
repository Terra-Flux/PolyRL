...
async def generate_request(obj: GenerateReqInput, request: Request):
    ...
    # polyrl-dev(liuxs): generate_request is not supported in async iterator mode
    assert not _global_state.tokenizer_manager.server_args.enable_async_server, "generate_request is not supported in async iterator mode"
    if obj.stream:
        ...
        try:
            # polyrl-dev(liuxs): gather results in the endpoint
            if _global_state.tokenizer_manager.server_args.enable_async_server:
                generator = _global_state.tokenizer_manager.generate_request(obj, request)
                # Collect all results using asyncio.gather
                outputs = []
...
# polyrl-dev
@app.api_route("/generate_async", methods=["POST", "PUT"])
async def generate_request_async(obj: GenerateReqInput, request: Request):
    """Handle a generate request. Return each request asynchronously when it's finished."""
    if obj.stream:
        raise NotImplementedError("Streaming is not supported for async requests")
    else:
        try:
            generator = _global_state.tokenizer_manager.generate_request(obj, request)
            
            async def collect_finished_requests() -> AsyncIterator[bytes]:
                async for out in generator:
                    rid = out["index"]
                    
                    logger.debug(f"Generated result for request {rid}: {out}")
                        
                    # Yield the complete response for this request
                    yield b"data: " + orjson.dumps(
                        out,
...
async def update_weights_from_agent(
    obj: UpdateWeightsFromAgentReqInput, request: Request
):
    """Update the weights from weight receiver agent."""
    success, message = await _global_state.tokenizer_manager.update_weights_from_agent(
        obj, request
    )
    content = {"success": success, "message": message}
    return ORJSONResponse(
...
def launch_server(
    ...
    # polyrl-dev: register this SGLang instance to rollout manager if provided
        ...
        if rollout_mgr_url:
            try:
                ...
                if res.status_code == 200:
                    ...
                    for k in [
                        ("mooncake_transfer_device_name", "mooncake_transfer_device_name"),
                        ("mooncake_transfer_protocol", "mooncake_transfer_protocol"),
