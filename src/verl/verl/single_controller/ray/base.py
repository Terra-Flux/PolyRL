...
# polyrl-dev
def get_pg_ip(pg: PlacementGroup):
    node_ip = {node["NodeID"]: node["NodeManagerAddress"] for node in ray.nodes()}
    specs = ray._private.state.state.placement_group_table(pg.id)
    # all bunles should be on the same node
    node_id = specs["bundles_to_node_id"][0]
    node_ip = node_ip[node_id]
    return node_ip
...
class RayResourcePool(ResourcePool):
    def __init__(
        ...
        # polyrl-dev
        # name_prefix is the name of the resource pool, e.g. global_pool
    ...
    def get_placement_groups(self, strategy="STRICT_PACK", name=None, device_name="cuda"):
        ...
        if device_name == "npu":
            ...
            device_name = "GPU"
        # polyrl-dev
        # A two-level list [num_gpus_per_node] * num_nodes
        # Each process uses one GPU and max_collocate_count CPUs
...
class RayWorkerGroup(WorkerGroup):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # polyrl-dev
        # Get already created workers directly from ray
    ...
    def _init_with_resource_pool(self, resource_pool, ray_cls_with_init, bin_pack, detached):
        ...
        for pg_idx, pg in enumerate(sort_placement_group_by_node_ip(pgs)):
            # Get the IP address of the placement group
            # polyrl-dev
            if rank == -1:
                root_pg_ip = get_pg_ip(pg)
                pg_ip = root_pg_ip
            else:
                pg_ip = get_pg_ip(pg)
            ...
            for local_rank in range(local_world_size):
                ...
                # polyrl-dev: WorkerDict
                cia_name = match.group(1) if match else cia_name  # "ActorClass(Obj)" -> "Obj"
                # e.g., 6vlxp9WorkerDict_0:0
                ...
                # create a worker
                # polyrl-dev
                # Here we actually create each worker
    ...
    def spawn(self, prefix_set):
        ...
        # polyrl-dev
        # The actual worker creation has already been completed in _init_with_resource_pool
        # Here we do not create workers, but get workers from ray
        # Because WorkerDict may bind all methods of multiple worker classes together
        # Here we separate the remote methods of different worker types into a dict

        # Remove the prefix of the worker class
...
def _bind_workers_method_to_parent(cls, key, user_defined_cls):
    ...
    for method_name in dir(user_defined_cls):
        ...
        if hasattr(method, MAGIC_ATTR):
            # polyrl-dev
            # cls is WorkerDict
            # user_defined_cls is the class wrapped by the ray ActorClass, which is the actual worker class
            # For example, ActorRolloutRefWorker, here we need to bind the method under it to WorkerDict
            # For example, method is compute_log_prob
            # The corresponding compute_log_prob.MAGIC_ATTR is a dict
            # It contains dispatch_mode, execute_mode, blocking

            def generate_function(name, key=key):
                def func(self, *args, **kwargs):
                    # dispatch to the actual worker
                    # polyrl-dev
                    # Dispatch to the actual worker class, which will be e.g., ActorRolloutRefWorker
                    # The class has already been extracted from the ray ActorClass wrapper
...
def create_colocated_worker_cls(class_dict: dict[str, RayClassWithInitArgs]):
    ...
    # polyrl-dev
    # class is the ray ActorClass
    # These worker classes need to share a base class
    # Here is an example of PPO:
    # cls_dict is:
    # {'actor_rollout': <verl.workers.fsdp_workers.ActorClass(ActorRolloutRefWorker) object at 0x7b76d7506
    # 020>, 'critic': <verl.workers.fsdp_workers.ActorClass(CriticWorker) object at 0x7b76d7506c50>, 'ref': <verl.workers.fsdp_work
    # ers.ActorClass(ActorRolloutRefWorker) object at 0x7b76d7507790>}   
    # worker_cls is: # <verl.single_controller.base.worker.Worker>
    ...
    class WorkerDict(worker_cls):
        # polyrl-dev
        # The init is done on the remote worker
        # So we need to extract the user_defined_cls wrapped by ray remote
        def __init__(self):
            super().__init__()
            self.worker_dict = {}
