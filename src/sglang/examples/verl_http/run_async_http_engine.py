...
from sglang.srt.entrypoints.verl_http_async_engine import HttpServerAsyncEngineAdapter
...
def run_batch_generation(
    engine: HttpServerAsyncEngineAdapter, 
    ...
    response_list = loop.run_until_complete(
        engine.async_generate(
            prompt=batch_prompts,  # because we have already convert it to prompt token id
...
def main():
    ...
    try:
        ...
        engine = HttpServerAsyncEngineAdapter(**engine_kwargs)
        ...
        loop.run_until_complete(engine.release_memory_occupation())
        
        import time
        time.sleep(4)
        
        print(f"\n===== Resuming Memory Occupation =====")
        loop.run_until_complete(engine.resume_memory_occupation())
        ...
        loop.run_until_complete(engine.flush_cache())
