...
from sglang.srt.entrypoints.http_engine import HttpEngineForRL
...
class VerlEngine:
    def __init__(
        ...
        nnodes: int = 1,
        backend: Literal["engine", "server", "http"] = "engine",
        **kwargs,
    ):
        # polyrl-dev
        if backend == 'engine' or (backend == 'server' and not kwargs.get('enable_weight_transfer_agent', False)):
        ...
        # NOTE(liuxs): tp size of http backend is provided by the remote server, this tp_size is used for weight update
        engine_kwargs = dict(
        ...
        # polyrl-dev
            ...
            if self._tp_rank == 0:
                self._engine = HttpEngineForRL(**engine_kwargs)
            else:
                self._engine = None
        else:
    ...
    def update_weights_from_agent(
        ...
        # tensors_meta is the metadata of the full tensor
        assert isinstance(self._engine, HttpServerEngineAdapter) \
            or isinstance(self._engine, HttpEngineForRL)

        if self._tp_rank == 0:
            self._engine.update_weights_from_agent(
