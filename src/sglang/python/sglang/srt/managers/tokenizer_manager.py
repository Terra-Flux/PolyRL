...
class TokenizerManager:
    ...
    async def _wait_one_response(
        ...
        while True:
            try:
                ...
                # polyrl-dev(liuxs): client should disconnect once it needs to update the weights or abort the request
    ...
    async def _handle_batch_request(
        ...
        if not is_stream:
            # polyrl-dev(liuxs): return any finished request instead of gathering all results
            if self.server_args.enable_async_server:
                rid_to_index = {rid: i for i, rid in enumerate(rids)}
                tasks = [gen.__anext__() for gen in generators]
                # polyrl-dev(liuxs): use coroutine to avoid blocking
    ...
    async def update_weights_from_agent(
        self,
        obj: UpdateWeightsFromAgentReqInput,
        request: Optional[fastapi.Request] = None,
    ) -> Tuple[bool, str]:
        self.auto_create_handle_loop()
        async with self.model_update_lock.writer_lock:
