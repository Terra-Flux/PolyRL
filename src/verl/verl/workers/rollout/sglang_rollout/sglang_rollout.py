    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                logger.info(f"[SGLangRollout] Rollout manager is ready at {endpoint}")
                return True
...
class SGLangRollout(BaseRollout):
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        if self.config.name == 'sglang-disaggregated':
            self._engine = None 
            ...
            if self._tp_rank == 0:
                ...
                if self._engine is not None:
                    ...
                    async def batch_generate():
                        # polyrl-dev
                        # TODO(liuxs): wrap this as a iterator
                        tasks = [
                            call_manager(idx_list[i], image_list[i]) for i in range(len(idx_list))
                        ]
                        return await asyncio.gather(*tasks)

                    output_list = loop.run_until_complete(batch_generate())
                    if do_sample and self.sampling_params.get("n", 1) > 1:
                        # NOTE: unroll the output if we are doing sampling
                        output = []
        ...
        for data_idx, raw_prompt in enumerate(prompts.non_tensor_batch["raw_prompt"]):
            for rollout_offset in range(n):
                if self._tool_schemas:
                    ...
                    # You are a math expert. You are given a question and you need to solve it step by step. Reasoning step by step before any tool call. You should use the `
                    # calc_gsm8k_reward` tool after step by step solving the question, before generate final answer at least once and refine your answer if necessary. Put your final answer in the for
                    # mat of `#### <answer>`
                    # # Tools
                    # You may call one or more functions to assist with the user query.
                    # You are provided with function signatures within <tools></tools> XML tags:
                    # <tools>
                    # {"type": "function", "function": {"name": "calc_gsm8k_reward",  ...
                    # </tools>
                    ...
                    # polyrl-dev
                    # Example:
                    # _tool_schemas: [OpenAIFunctionToolSchema(
                    #   type='function', 
                    ...
                    #       parameters=OpenAIFunctionParametersSchema(type='object', properties={'answer': OpenAIFunctionPropertySchema(type='string', description="The model's answer to the GSM8K math problem, must be a digits", enum=None)}, required=['answer']), strict=False))], 
                    # _tools_kwargs: {'calc_gsm8k_reward': {'create_kwargs': {'ground_truth': '15'}}}
